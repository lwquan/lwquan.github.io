...
class OrderLogic
{
	...
	public function addOrder($user_id,$address_id,$shipping_code,$invoice_title = "",$coupon_id = 0,$car_price,$user_note='',$pay_name)
	{
		...
		if($car_price['pointsFee']>0)
			M('users')->where("user_id", $user_id)->setDec('pay_points',($car_price['pointsFee'] * (tpCache('shopping.point_rate')/100))); // 消费积分
	...
	public function addOrder_new($user_id,$address_id,$shipping_code,$invoice_title = "",$coupon_id = 0,$car_price,$user_note='',$pay_name)
    {
        ...
        // 3 扣除积分 扣除余额
        if($car_price['pointsFee']>0)
            M('Users')->where("user_id", $user_id)->setDec('pay_points',(($car_price['pointsFee']))); // 消费积分
        if($car_price['balance']>0)
            M('Users')->where("user_id", $user_id)->setDec('user_money',$car_price['balance']); // 抵扣余额
        // 4 删除已提交订单商品
        M('Cart')->where(['user_id' => $user_id,'selected' => 1])->delete();
