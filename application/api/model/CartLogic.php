...
class CartLogic extends Model
{
    ...
    {
        parent::__construct();
        $this->user_id = $_SERVER['HTTP_USER_ID']?$_SERVER['HTTP_USER_ID']:0;
    ...
    public function addGoodsToCart()
    {
        ...
        return array('status' => -9, 'msg' => '购物车最多只能放20种商品', 'result' => $result);
    }
    ...
    private function addNormalCart(){
        ...
        if ($userCartGoods) {
            ...
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum,'goods_price'=>$price,'member_goods_price'=>$price,'selected' => 1]);
        }else{
            //如果该商品没有存在购物车
    ...
    private function addFlashSaleCart(){
        ...
        // 如果该商品已经存在购物车
        if($userCartGoods){
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum,'selected' => 1]);
        }else{
