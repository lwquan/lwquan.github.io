...
class GoodsLogic extends Model
{
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                $parr[] = array('value' => "{$end}元以下", 'key' =>  $filter_param['price']);
            elseif($i == ($c-1) && ($max_price > $end))
                $parr[] = array('value' => "{$end}元以上", 'key' =>  $filter_param['price']);
            else
                $parr[] = array('value' => "{$start}-{$end}元", 'key' =>  $filter_param['price']);
        }
        return $parr;
    }
     ...
     * @return mixed
     */
    function getFilterTypeSelected($typeArr ,$cat_id = array(1)){
