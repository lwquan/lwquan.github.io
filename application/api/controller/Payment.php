...
//        $this->ajaxReturn(['status'=>1,'msg'=>'支付成功','result'=>$res]);
    ...
    public function getCode(){
            ...
            if(!session('user'))$this->ajaxReturn(['status'=>1,'msg'=>'请先登录']);
            // 修改订单的支付方式
           ...
           if($this->pay_code == 'weixin' && $_SESSION['openid'] && strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            ...
            update_pay_status($order['order_sn'],array('transaction_id'=>$order['order_sn'])); // 修改订单支付状态
            $this->ajaxReturn(['status'=>1,'msg'=>'支付成功','result'=>$order]);
...
//        $this->ajaxReturn(['status'=>1,'msg'=>'充值成功','result'=>$user]);
        if($order_id>0){
...
//                    $this->ajaxReturn(['status'=>1,'msg'=>'充值成功','result'=>$config_value]);
        			$code_str = $this->payment->get_code($order,$config_value);
        		}
        	}else{
